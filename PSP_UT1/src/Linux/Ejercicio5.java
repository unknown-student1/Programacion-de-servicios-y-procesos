...
public class Ejercicio5 {
    ...
    public static void main(String[] args) {
        try {
            ...
            Process process = Runtime.getRuntime().exec("ls -al /usr/bin");
            ...
            while ((line = reader.readLine()) != null) {
                // Añade los tamaños de los archivos a la lista
            ...
            // Muestra los resultados en bytes
            System.out.println("Total size: " + totalSize + " bytes");
            System.out.println("Largest file size: " + maxSize + " bytes");

            // Muestra los resultados en MB
    ...
    public static ArrayList<Long> Columna(String input) {
        ...
        if (!input.trim().isEmpty()) {  // Asegura que la línea no esté vacía
            ...
            if (parts.length > 4) { // Asegura que haya suficientes partes en la línea
                try {
                    // El tamaño del archivo está en la quinta columna
                    long size = Long.parseLong(parts[4]);
                    fileSizes.add(size);
                } catch (NumberFormatException e) {
                    // Imprime un mensaje de error si no se puede convertir el tamaño del archivo a un número
